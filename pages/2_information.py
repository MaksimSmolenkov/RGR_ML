...
data2= pd.read_csv("classification_data.csv")
df2 = pd.DataFrame(data2)

st.title('Информация о датасетe')

st.header('Датасет для классификации - "Срабатывания датчика дыма"')
st.subheader('    Датасет посвящен исследованию пожароопасных ситуций, а также поведение датчика дыма в различных условиях')
st.markdown('Рассматривается для задачи классификации')
st.markdown('---')
st.dataframe(df2)
